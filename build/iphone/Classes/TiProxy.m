...
//Rare exceptions to indicate a bug in the _europom code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_europom = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_europom]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.europom.module"
